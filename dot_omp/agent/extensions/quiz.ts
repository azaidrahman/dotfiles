...
// ────────────────────────────────────────────────────────────────────────────
// quiz — a GRADED sibling of ask.
//
// Where ask collects a preference/decision with no notion of
...
export default function quiz(pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "quiz",
		description:
			"Ask the user a GRADED question with a known correct answer, then instantly grade and give feedback. Unlike ask (which collects preferences/decisions with no right answer), quiz always has a correct answer supplied by you, marks the user's selection right/wrong (✓/✗), reveals the correct answer, and can show an explanation. Use it to (1) assess what the learner already understands before teaching, and (2) run tight practice/retrieval loops after explaining, or probe understanding whenever you're unsure they've got it. Options-only: single-select or multi-select, plus an automatic 'I don't know' choice so the user can signal a genuine gap instead of guessing. An always-present optional note field (Tab to focus it) lets the user attach a free-text note to ANY answer; it reaches you only when non-empty. No free-text answers — for non-graded questions use ask instead.",
		promptSnippet:
			"Use the quiz tool to test the user with a graded multiple-choice or multi-select question (required correct answer + required explanation). For non-graded questions, use ask.",
		promptGuidelines: [
			"For a teach lesson, use quiz for every question that has a right answer, as the teach skill says.",
			"quiz is GRADED; ask is not. If the question has a correct answer, use quiz. If you just need a preference, decision, or open-ended input, use ask.",
