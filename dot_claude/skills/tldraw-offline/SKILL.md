...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/Users/zaid/Library/Application Support/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/Users/zaid/Library/Application Support/tldraw/server.json'); TOKEN=$(jq -r .token '/Users/zaid/Library/Application Support/tldraw/server.json')
